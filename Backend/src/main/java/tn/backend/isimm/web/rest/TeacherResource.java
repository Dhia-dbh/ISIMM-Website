...
import tn.backend.isimm.repository.TeacherRepository;
import tn.backend.isimm.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link tn.backend.isimm.domain.Teacher}.
 */
@RestController
...
public class TeacherResource {
    ...
    public ResponseEntity<Teacher> createTeacher(@Valid @RequestBody Teacher teacher) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/teachers/" + teacher.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, teacher.getId().toString()))
            .body(teacher);
    }
    ...
    public ResponseEntity<Teacher> partialUpdateTeacher(
        ...
        Optional<Teacher> result = teacherRepository
            ...
            .map(existingTeacher -> {
                if (teacher.getCin() != null) {
                    existingTeacher.setCin(teacher.getCin());
                }
                if (teacher.getNumTeacher() != null) {
                    existingTeacher.setNumTeacher(teacher.getNumTeacher());
                }
